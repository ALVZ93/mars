...
function codexScript(): string {
  ...
  catch { throw new MarsError('ConfigurationError', 'The bundled Codex runtime is unavailable. Reinstall MARS with optional platform dependencies enabled.'); }
}
...
async function runCodex(args: string[], signal: AbortSignal, inherit = false): Promise<{ code: number; output: string }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('error', error => {
      ...
      reject(new MarsError('ConfigurationError', `Could not start the bundled Codex runtime: ${error.message}`));
    });
...
export class OpenAICodexAuthProvider implements AuthProvider {
  ...
  async login(method: AuthMethod, context: AuthContext): Promise<ExternalCredential> {
    if (method !== 'oauth-pkce' && method !== 'oauth-device') throw new MarsError('ConfigurationError', 'OpenAI Codex supports browser or device-code login.');
    ...
    if (!status.authenticated) {
      ...
      if (result.code !== 0) throw new MarsError('AuthenticationError', 'Codex login did not complete successfully.');
      status = await this.status(undefined, context);
    }
    if (!status.authenticated) throw new MarsError('AuthenticationError', 'Codex did not report an authenticated ChatGPT session.');
  ...
  async logout(_credential: Credential, context: AuthContext): Promise<void> {
    ...
    if (result.code !== 0) throw new MarsError('AuthenticationError', 'Codex logout did not complete successfully.');
  }
